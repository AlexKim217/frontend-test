...
Здесь содержится информация о пользователе, текст о себе, имя пользователя.
...
- Основная библиотека - `Vue3`.
- Компонентная библиотека - `PrimeVue`
- Для старта можно использовать `Vue CLI`
- В ходе разработки необходимо добавить обработку ошибок, вывод нотисов, вывод ошибок валлидации.

## Запуск API

1. Убедитесь, что у вас стоит последняя node LTS версии. Ссылка для скачивания: https://nodejs.org/en/
2. Устанавливаем зависимости api - в консоли запускаем команду `npm install`
3. Запускаем api - в консоли запускаем команду `node app.js`

В результате в консоли получаем output:

```
Сервер с API стартовал по адресу http://localhost:8081
```
...
1. Во время разработки приложения не стоит изобретать велосипеды. Для компонентов формы и карточек предусмотрена библиотека `primeVue`.
1. При работе с `Vue` следует использовать `Vue Router`. State-manager на ваше усмотрение.
...
- По умолчанию api запускается по адресу `http://localhost:8081`
